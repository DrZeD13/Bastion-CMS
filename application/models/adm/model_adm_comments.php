...
class Model_Adm_Comments extends Model 
{
	...
	var $orderType = array ("comment_id"=>"", "name" =>"", "ip" =>"", "news_date"=>"", "is_active" => "", "module" => "", "comment" => "");
	...
	{					
		...
		{			
			...
			$sql="SELECT {$this->primary_key}, news_date, name, comment, ip, is_active, new, module, parent_id ".$fromwhere;			
			...
			{				
				...
				switch ($module)
				{
					case "products":
						...
						$linktitle = $this->db->GetAdminTitle("products");
					break;
					case "articles":
						$sql = "SELECT url FROM articles WHERE article_id = '$parent_id'";
						$url = $this->db->GetOne($sql);
						$link = "/".ARTICLES_LINK."/".$url;
						$linktitle = $this->db->GetAdminTitle("articles");
					break;
				}
				$data ["row"][] = array (
					"id" => $id,
					"name" => $this->dec($row['name']),
					"comment" => mb_substr($this->dec($row['comment']), 0, 70),
					"ip" => $this->dec($row['ip']),
